...
export class GameEnvironment {
  ...
  calculateRewards() {
    ...
    this.redTeam.forEach(agent => {
      ...
      if (agent.hasFlag) {
        // Has enemy flag
        reward += 0.5 * this.rewardWeights.offense;
        
        // Moving towards own base with flag
        if (agent.x < this.gridSize / 2) {
          reward += (this.gridSize/2 - agent.x) * 0.1 * this.rewardWeights.offense;
        }
        ...
        reward += (20 - distToFlag) * 0.02 * this.rewardWeights.offense;
      }
      
      // Defensive rewards
      if (agent.x < this.gridSize / 2) { // In own territory
        // Reward for being close to own flag
        const distToFlag = Math.abs(agent.x - this.redFlag.x) + Math.abs(agent.y - this.redFlag.y);
        if (distToFlag < 5) {
          reward += (5 - distToFlag) * 0.05 * this.rewardWeights.defense;
        }
        ...
        if (blueWithFlag && blueWithFlag.x < this.gridSize / 2) {
          ...
          reward += (10 - distToEnemy) * 0.1 * this.rewardWeights.defense;
        }
      }
      ...
      if (numTeammatesNearby > 0) {
        // Small reward for team formation
        reward += numTeammatesNearby * 0.05 * this.rewardWeights.cooperation;
      }
      
      // Big rewards for scoring
      if (this.redScore > 0 && this.redFlagHolder === null) {
        reward += 10.0 * this.rewardWeights.offense;
      }
    ...
    this.blueTeam.forEach(agent => {
      ...
      if (agent.hasFlag) {
        ...
        // Moving towards own base with flag
        if (agent.x > this.gridSize / 2) {
          reward += (agent.x - this.gridSize/2) * 0.1 * this.rewardWeights.offense;
        }
      } else if (agent.x < this.gridSize / 2) {
        // In enemy territory, closer to enemy flag
        const distToFlag = Math.abs(agent.x - this.redFlag.x) + Math.abs(agent.y - this.redFlag.y);
